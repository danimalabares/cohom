...
\begin{prop}
		\[H^*(\R P^n,\Z/2)\approx\bigoplus_{i=0}^n\Z/2\approx\Z/2[X]/\langle X^{n+1}\rangle\]
		Para lo cual basta ver que el producto de los elementos no triviales en cada grado es el producto no trivial en la suma de los grados.
\end{prop}
\begin{proof}
	Recordemos que
	\begin{align*}
	...
	Consideremos los encajes naturales
	\begin{align*}
		\begin{aligned}
	...
	\[\begin{tikzcd}
		...
		H^i(P^n,P^n-P^j)\times(P^n,P^n-P^i)\arrow[u]\arrow[r,"\smile"]\arrow[d]&H^n(P^n,P^n-\{p\})\arrow[u]\arrow[d]\\
	...
	Luego, la cohomología del producto tensorial de $\tilde{H}^*(S^{j-1})\otimes \tilde{H}^*(S^{i-1})$ es justamente $\Z/2\otimes\Z/2\approx\Z/2$ ya que son los únicos que factores que sobreviven en todos los grados. Finalmente aplicamos Künneth en el renglón de abajo.
\end{proof}
\begin{obs}
	Podemos pensar que el producto copa mide cómo se intersectan dos subvariedades.
\end{obs}
\begin{obs}
	Fi existe $f:X\to\R P^n$, entonces la función inducida $f^*:H^*(\R P^n;\Z/2)\to H^*(X;\Z/2)$ está determinada por su acción en el polinomio $x$.
\end{obs}
\end{document}